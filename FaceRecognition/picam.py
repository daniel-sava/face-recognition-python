...
class OpenCVCapture(object):
    def read(self):
        """Read a single frame from the camera and return the data as an OpenCV image (which is a numpy array)."""
        ...
        with picamera.PiCamera() as camera:
            camera.capture(data, format='png')

        data = np.fromstring(str(data.getvalue()), dtype=np.uint8)
